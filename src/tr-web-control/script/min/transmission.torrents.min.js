...
a.downloadDir.split("/");var j="folders-",h;for(h in e){var f=e[h];if(f!=""){j+=d.encode(f);(f=this.folders[j])||(f={count:0,torrents:[],size:0,nodeid:j});f.torrents.push(a);f.count++;f.size+=a.totalSize;this.folders[j]=f}}}this.count++}}transmission.downloadDirs=transmission.downloadDirs.sort();this.newIds.length>0&&this.getallids(null,this.newIds)},addTracker:function(b){var d=b.trackerStats,c=false,a=[];b.leecherCount=0;b.seederCount=0;if(d.length>0){for(var g in d){var e=d[g],j=e.lastAnnounceResult.toLowerCase(),
h=e.host.getHostName().split(".");$.inArray(h[0],"www,tracker".split(","))!=-1&&h.shift();h=h.join(".");var f="tracker-"+h.replace(/\./g,"-"),i=transmission.trackers[f];if(!i){transmission.trackers[f]={count:0,torrents:[],size:0,connected:true,isBT:d.length>5};i=transmission.trackers[f]}i.name=h;i.nodeid=f;i.host=e.host;if(!e.lastAnnounceSucceeded&&e.announceState!=transmission._trackerStatus.inactive){c=true;b.warning=e.lastAnnounceResult;if(j=="could not connect to tracker")i.connected=false}if(i.torrents.indexOf(b)==
-1){i.torrents.push(b);i.count++;i.size+=b.totalSize}b.leecherCount+=e.leecherCount;b.seederCount+=e.seederCount;a.indexOf(h)==-1&&a.push(h)}d.length>5&&this.btItems.push(b);if(c){if(b.nextAnnounceTime){if(b.nextAnnounceTime>e.nextAnnounceTime)b.nextAnnounceTime=e.nextAnnounceTime}else b.nextAnnounceTime=e.nextAnnounceTime;this.warning.push(b)}if(b.leecherCount<0)b.leecherCount=0;if(b.seederCount<0)b.seederCount=0;b.leecher=b.leecherCount+" ("+b.peersGettingFromUs+")";b.seeder=b.seederCount+" ("+
b.peersSendingToUs+")";b.trackers=a.join(";")}},getPeers:function(b){transmission.exec({method:"torrent-get",arguments:{fields:"peers,peersFrom".split(","),ids:b}},function(d){console.log("data:",d)})},getMoreInfos:function(b,d,c){transmission.exec({method:"torrent-get",arguments:{fields:b.split(","),ids:d}},function(a){if(a.result=="success")c&&c(a.arguments.torrents);else c&&c(null)})},search:function(b,d){if(!b)return null;if(!d)d=this.all;var c=[];$.each(d,function(a){d[a].name.toLowerCase().indexOf(b.toLowerCase())!=
...
e.status!=transmission._status.stopped&&c.push(e.id)}}for(g in this.warning){e=this.warning[g];if(!($.inArray(e.id,b)!=-1&&b.length>0)){if(d==true)if(a<e.nextAnnounceTime)continue;c.push(e.id)}}return c},searchAndReplaceTrackers:function(b,d,c){if(b&&d){var a={},g=0,e;for(e in this.all){var j=this.all[e];if(!j)return;var h=j.trackerStats,f;for(f in h)if(h[f].announce==b){a[f]||(a[f]={ids:[],tracker:d});a[f].ids.push(j.id);g++}}g==0&&c&&c(null,0);for(e in a)transmission.exec({method:"torrent-set",
arguments:{ids:a[e].ids,trackerReplace:[parseInt(e),a[e].tracker]}},function(i,k){if(i.result=="success")c&&c(k,g);else c&&c(null)},a[e].ids)}}};
