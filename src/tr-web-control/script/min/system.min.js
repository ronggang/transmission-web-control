...
a.id){var b=a.id,c=a.options,d=a.datas,f=$("#"+b);if(f.length){d&&$.each(d,function(a,b){f.data(a,b)});if(0==a.type&&f.attr("type")==a.type){f.dialog("open");f.dialog({content:system.templates[b]});return}if(0!=system.popoverCount){setTimeout(function(){system.openDialogFromTemplate(a)},350);return}f.remove()}c=$.extend(!0,{title:"",width:100,height:100,resizable:!1,cache:!0,content:system.lang.dialog["system-config"].loading,modal:!0},c);f=$("\x3cdiv/\x3e").attr({id:b,type:a.type}).appendTo(document.body);
0==a.type?f.dialog(c):(f.css({width:c.width,height:c.height}).data("popoverSource",a.source),$(a.source).webuiPopover({url:"#"+b,width:c.width,height:c.height-18,padding:!1,onHide:function(c){$(a.source).webuiPopover("destroy");$("#"+b).remove();$(c).remove();system.popoverCount--},onShow:function(){system.popoverCount++}}));$.get(system.rootPath+"template/"+b+".html?time\x3d"+new Date,function(c){system.templates[b]=c;d&&$.each(d,function(a,c){$("#"+b).data(a,c)});0==a.type?$("#"+b).dialog({content:c}):
(f.html(c),$.parser.parse("#"+b),$(a.source).webuiPopover("show"))})}},debug:function(a,b){window.console&&window.console.log&&window.console.log(a,b)},initThemes:function(){this.themes&&$("#select-themes").combobox({groupField:"group",data:this.themes,editable:!1,panelHeight:"auto",onChange:function(a){var b=(a+";").split(";"),c=b[0];b=b[1]||"logo.png";$("#styleEasyui").attr("href","tr-web-control/script/easyui/themes/"+c+"/easyui.css");$("#logo").attr("src","tr-web-control/"+b);system.config.theme=
