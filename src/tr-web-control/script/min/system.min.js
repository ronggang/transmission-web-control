...
this.panel.left.tree("find",a);switch(a){case "folders":if(system.config.foldersShow){$("#tree-toolbar-nav-"+a).click();$(b.target).parent().show()}else $(b.target).parent().hide()}}if(this.config.defaultSelectNode)(a=this.panel.left.tree("find",this.config.defaultSelectNode))&&this.panel.left.tree("select",a.target)},initTorrentTable:function(){function a(){b&&$(b).remove();b=$("<div/>").appendTo("body");b.menu({onClick:function(i){if(i.iconCls=="icon-ok"){system.control.torrentlist.datagrid("hideColumn",
i.name);b.menu("setIcon",{target:i.target,iconCls:"icon-empty"})}else{system.control.torrentlist.datagrid("showColumn",i.name);b.menu("setIcon",{target:i.target,iconCls:"icon-ok"})}system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.saveUserConfig()}});for(var d=system.control.torrentlist.datagrid("getColumnFields"),f=0;f<d.length;f++){var g=d[f],h=system.control.torrentlist.datagrid("getColumnOption",g);if(h.allowCustom!=false&&h.allowCustom!=
"false")b.menu("appendItem",{text:h.title,name:g,iconCls:h.hidden?"icon-empty":"icon-ok"})}}this.control.torrentlist=$("<table/>").attr("class","torrent-list").appendTo(this.panel.list);var b=null,c=-1,e=false;$.get(system.rootPath+"template/torrent-fields.json?time="+new Date,function(d){d=d.fields;if(system.userConfig.torrentList.fields.length!=0)d=$.extend(d,system.userConfig.torrentList.fields);var f=JSON.stringify(d);system.userConfig.torrentList.fields=JSON.parse(f);for(var g in d){d[g].title=
system.lang.torrent.fields[d[g].field]||d[g].field;system.setFieldFormat(d[g])}system.control.torrentlist.datagrid({autoRowHeight:false,pagination:system.config.pagination,rownumbers:true,remoteSort:false,checkOnSelect:false,pageSize:system.config.pageSize,pageList:system.config.pageList,idField:"id",fit:true,striped:true,sortName:system.userConfig.torrentList.sortName,sortOrder:system.userConfig.torrentList.sortOrder,drophead:true,columns:[d],onCheck:function(h,i){system.checkTorrentRow(h,i)},onUncheck:function(h,
i){system.checkTorrentRow(h,i)},onCheckAll:function(){system.checkTorrentRow("all",false)},onUncheckAll:function(){system.checkTorrentRow("all",true)},onSelect:function(h,i){if(c!=-1){e=true;system.control.torrentlist.datagrid("unselectRow",c);e=false}system.config.autoExpandAttribute&&system.panel.attribute.panel("options").collapsed&&system.panel.layout_body.layout("expand","south");system.getTorrentInfos(i.id);c=h},onUnselect:function(){if(system.config.autoExpandAttribute)if(e==false)system.panel.attribute.panel("options").collapsed||
system.panel.layout_body.layout("collapse","south");system.currentTorrentId=0;c=-1},onBeforeLoad:function(){system.currentTorrentId=0},onSortColumn:function(h,i){var k=h;if(h=="remainingTime")k="remainingTimeRaw";k=system.control.torrentlist.datagrid("getData").originalRows.sort(arrayObjectSort(k,i));system.control.torrentlist.datagrid("loadData",k);system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.userConfig.torrentList.sortName=h;system.userConfig.torrentList.sortOrder=
...
b){window.localStorage[a]=b},openDialogFromTemplate:function(a){a=$.extend(true,{id:null,options:null,datas:null},a);if(a.id!=null){var b=a.id,c=a.options,e=a.datas,d=$("#"+b);if(d.length){d.dialog("open");e&&$.each(e,function(f,g){d.data(f,g)});d.dialog({content:system.templates[b]})}else{c=$.extend(true,{title:"",width:100,height:100,resizable:false,cache:true,content:"loading...",modal:true},c);$("<div/>").attr("id",b).appendTo(document.body).dialog(c);$.get(system.rootPath+"template/"+b+".html?time="+
new Date,function(f){system.templates[b]=f;e&&$.each(e,function(g,h){$("#"+b).data(g,h)});$("#"+b).dialog({content:f})})}}},debug:function(a,b){window.console&&window.console.log&&window.console.log(a,b)},initThemes:function(){this.themes&&$("#select-themes").combobox({groupField:"group",data:this.themes,editable:false,panelHeight:"auto",onChange:function(a){var b=(a+";").split(";"),c=b[0];b=b[1]||"logo.png";$("#styleEasyui").attr("href","tr-web-control/script/easyui/themes/"+c+"/easyui.css");$("#logo").attr("src",
