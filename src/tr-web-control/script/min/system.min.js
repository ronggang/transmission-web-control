...
0},onSortColumn:function(field,order){var field_func=field;if(field=="remainingTime")field_func="remainingTimeRaw";var datas=system.control.torrentlist.datagrid("getData").originalRows.sort(arrayObjectSort(field_func,order));system.control.torrentlist.datagrid("loadData",datas);system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.userConfig.torrentList.sortName=field;system.userConfig.torrentList.sortOrder=order;system.saveUserConfig()},onRowContextMenu:function(e,
rowIndex,rowData){system.control.torrentlist.datagrid("uncheckAll");system.control.torrentlist.datagrid("checkRow",rowIndex);e.preventDefault();system.showContextMenu("torrent-list",e)},onHeadDrop:function(sourceField,targetField){system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.saveUserConfig()},onResizeColumn:function(field,width){system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.saveUserConfig()},
...
this.lang.toolbar.tip["start-all"]).click(function(){var button=$(this);var icon=button.linkbutton("options").iconCls;button.linkbutton({disabled:true,iconCls:"icon-loading"});transmission.exec({method:"torrent-start"},function(data){button.linkbutton({iconCls:icon,disabled:false});button=null})});this.panel.toolbar.find("#toolbar_pause_all").linkbutton({disabled:false}).attr("title",this.lang.toolbar.tip["pause-all"]).click(function(){var button=$(this);var icon=button.linkbutton("options").iconCls;
button.linkbutton({disabled:true,iconCls:"icon-loading"});transmission.exec({method:"torrent-stop"},function(data){button.linkbutton({iconCls:icon,disabled:false});button=null})});this.panel.toolbar.find("#toolbar_start").linkbutton({disabled:true}).attr("title",this.lang.toolbar.tip["start"]).click(function(){system.changeSelectedTorrentStatus("start",$(this))});this.panel.toolbar.find("#toolbar_pause").linkbutton({disabled:true}).attr("title",this.lang.toolbar.tip["pause"]).click(function(){system.changeSelectedTorrentStatus("stop",
$(this))});this.panel.toolbar.find("#toolbar_recheck").linkbutton({disabled:true}).attr("title",this.lang.toolbar.tip["recheck"]).click(function(){var rows=system.control.torrentlist.datagrid("getChecked");if(rows.length>0)if(rows.length==1){var torrent=transmission.torrents.all[rows[0].id];if(torrent.percentDone>0){if(confirm(system.lang.toolbar.tip["recheck-confirm"]))system.changeSelectedTorrentStatus("verify",$(this))}else system.changeSelectedTorrentStatus("verify",$(this))}else if(confirm(system.lang.toolbar.tip["recheck-confirm"]))system.changeSelectedTorrentStatus("verify",
$(this))});this.panel.toolbar.find("#toolbar_morepeers").linkbutton({disabled:true}).click(function(){system.changeSelectedTorrentStatus("reannounce",$(this))});this.panel.toolbar.find("#toolbar_remove").linkbutton({disabled:true}).attr("title",this.lang.toolbar.tip["remove"]).click(function(){var rows=system.control.torrentlist.datagrid("getChecked");var ids=new Array;for(var i in rows)ids.push(rows[i].id);if(ids.length==0)return;system.openDialogFromTemplate({id:"dialog-torrent-remove-confirm",
options:{title:system.lang.dialog["torrent-remove"].title,width:350,height:150},datas:{"ids":ids}})});this.panel.toolbar.find("#toolbar_rename").linkbutton({disabled:true}).click(function(){var rows=system.control.torrentlist.datagrid("getChecked");if(rows.length==0)return;system.openDialogFromTemplate({id:"dialog-torrent-rename",options:{title:system.lang.dialog["torrent-rename"].title,width:450,height:150,resizable:true},datas:{id:rows[0].id}})});this.panel.toolbar.find("#toolbar_changeDownloadDir").linkbutton({disabled:true}).attr("title",
this.lang.toolbar.tip["change-download-dir"]).click(function(){var rows=system.control.torrentlist.datagrid("getChecked");var ids=new Array;for(var i in rows)ids.push(rows[i].id);if(ids.length==0)return;system.openDialogFromTemplate({id:"dialog-torrent-changeDownloadDir",options:{title:system.lang.dialog["torrent-changeDownloadDir"].title,width:520,height:200},datas:{"ids":ids}})});this.panel.toolbar.find("#toolbar_alt_speed").linkbutton().attr("title",this.lang.toolbar.tip["alt-speed"]).click(function(){var button=
$(this);var options=button.linkbutton("options");var enabled=false;if(options.iconCls=="icon-alt-speed-false")enabled=true;transmission.exec({method:"session-set",arguments:{"alt-speed-enabled":enabled}},function(data){if(data.result=="success"){system.serverConfig["alt-speed-enabled"]=enabled;button.linkbutton({iconCls:"icon-alt-speed-"+enabled.toString()});if(enabled)$("#status_alt_speed").show();else $("#status_alt_speed").hide()}});button.linkbutton({iconCls:"icon-loading"})});this.panel.toolbar.find("#toolbar_config").linkbutton().attr("title",
this.lang.toolbar.tip["system-config"]).click(function(){system.openDialogFromTemplate({id:"dialog-system-config",options:{title:system.lang.toolbar["system-config"],width:620,height:440,resizable:true}})});this.panel.toolbar.find("#toolbar_reload").linkbutton().attr("title",this.lang.toolbar.tip["system-reload"]).click(function(){system.reloadData()});this.panel.toolbar.find("#toolbar_search").searchbox({searcher:function(value){system.searchTorrents(value)},prompt:this.lang.toolbar["search-prompt"]})},
...
reloadSession:function(isinit){transmission.getSession(function(result){system.serverConfig=result;$("#status_version").html("Transmission "+system.lang.statusbar.version+result["version"]+", RPC: "+result["rpc-version"]+", WEB Control: "+system.version+"("+system.codeupdate+")");if(result["alt-speed-enabled"]==true){system.panel.toolbar.find("#toolbar_alt_speed").linkbutton({iconCls:"icon-alt-speed-true"});$("#status_alt_speed").show()}else{system.panel.toolbar.find("#toolbar_alt_speed").linkbutton({iconCls:"icon-alt-speed-false"});
$("#status_alt_speed").hide()}system.downloadDir=result["download-dir"];if(transmission.downloadDirs.length==0)transmission.downloadDirs.push(system.downloadDir);if(parseInt(system.serverConfig["rpc-version"])>=15)transmission.getFreeSpace(system.downloadDir,function(datas){system.serverConfig["download-dir-free-space"]=datas.arguments["size-bytes"];system.showFreeSpace(datas.arguments["size-bytes"])});else system.showFreeSpace(system.serverConfig["download-dir-free-space"]);if(isinit)system.showStatus(system.lang.system.status.connected)})},
showFreeSpace:function(size){var tmp=size;if(tmp==-1)tmp=system.lang["public"]["text-unknown"];else tmp=formatSize(tmp);$("#status_freespace").text(system.lang.dialog["system-config"]["download-dir-free-space"]+" "+tmp)},reloadTorrentBaseInfos:function(ids,moreFields){if(this.reloading)return;clearTimeout(this.autoReloadTimer);this.reloading=true;var oldInfos={trackers:transmission.trackers,folders:transmission.torrents.folders};transmission.torrents.getallids(function(resultTorrents){var ignore=
new Array;for(var index in resultTorrents){var item=resultTorrents[index];ignore.push(item.id)}var errorIds=transmission.torrents.getErrorIds(ignore,true);if(errorIds.length>0)transmission.torrents.getallids(function(){system.resetTorrentInfos(oldInfos)},errorIds);else system.resetTorrentInfos(oldInfos)},ids,moreFields)},resetTorrentInfos:function(oldInfos){var currentTorrentId=this.currentTorrentId;var parentNode=this.panel.left.tree("find","servers");if(parentNode){var parentNode_collapsed=parentNode.state;
this.removeTreeNode("servers-loading")}else{this.appendTreeNode(null,[{id:"servers",text:this.lang.tree.servers,state:"closed",iconCls:"icon-servers"}]);parentNode=this.panel.left.tree("find","servers")}var datas=new Array;for(var index in transmission.trackers){var tracker=transmission.trackers[index];var node=system.panel.left.tree("find",tracker.nodeid);var text=tracker.name+this.showNodeMoreInfos(tracker.count,tracker.size);if(node)system.updateTreeNodeText(tracker.nodeid,text,tracker.connected?
"icon-server":"icon-server-error");else system.appendTreeNode(parentNode,[{id:tracker.nodeid,text:text,iconCls:tracker.connected?"icon-server":"icon-server-error"}]);oldInfos.trackers[tracker.nodeid]=null}if(parentNode_collapsed=="closed")this.panel.left.tree("collapse",parentNode.target);for(var index in oldInfos.trackers){var tracker=oldInfos.trackers[index];if(tracker)system.removeTreeNode(tracker.nodeid)}if(transmission.torrents.status[transmission._status.stopped])system.updateTreeNodeText("paused",
system.lang.tree.paused+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.stopped].length));else system.updateTreeNodeText("paused",system.lang.tree.paused);if(transmission.torrents.status[transmission._status.seed])system.updateTreeNodeText("sending",system.lang.tree.sending+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.seed].length));else system.updateTreeNodeText("sending",system.lang.tree.sending);if(transmission.torrents.status[transmission._status.seedwait]){var node=
system.panel.left.tree("find","sending");var childs=system.panel.left.tree("getChildren",node.target);var text=system.lang.tree.wait+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.seedwait].length);if(childs.length>0)system.updateTreeNodeText(childs[0].id,text);else system.appendTreeNode(node,[{id:"seedwait",text:text,iconCls:"icon-wait"}])}else system.removeTreeNode("seedwait");if(transmission.torrents.status[transmission._status.check])system.updateTreeNodeText("check",
system.lang.tree.check+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.check].length));else system.updateTreeNodeText("check",system.lang.tree.check);if(transmission.torrents.status[transmission._status.checkwait]){var node=system.panel.left.tree("find","check");var childs=system.panel.left.tree("getChildren",node.target);var text=system.lang.tree.wait+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.checkwait].length);if(childs.length>0)system.updateTreeNodeText(childs[0].id,
text);else system.appendTreeNode(node,[{id:"checkwait",text:text,iconCls:"icon-wait"}])}else system.removeTreeNode("checkwait");if(transmission.torrents.status[transmission._status.download])system.updateTreeNodeText("downloading",system.lang.tree.downloading+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.download].length));else system.updateTreeNodeText("downloading",system.lang.tree.downloading);if(transmission.torrents.status[transmission._status.downloadwait]){var node=
system.panel.left.tree("find","downloading");var childs=system.panel.left.tree("getChildren",node.target);var text=system.lang.tree.wait+this.showNodeMoreInfos(transmission.torrents.status[transmission._status.downloadwait].length);if(childs.length>0)system.updateTreeNodeText(childs[0].id,text);else system.appendTreeNode(node,[{id:"downloadwait",text:text,iconCls:"icon-wait"}])}else system.removeTreeNode("downloadwait");system.updateTreeNodeText("actively",system.lang.tree.actively+this.showNodeMoreInfos(transmission.torrents.actively.length));
system.updateTreeNodeText("error",system.lang.tree.error+this.showNodeMoreInfos(transmission.torrents.error.length));system.updateTreeNodeText("warning",system.lang.tree.warning+this.showNodeMoreInfos(transmission.torrents.warning.length));var node=system.panel.left.tree("getSelected");if(node!=null){var p=system.control.torrentlist.datagrid("options").pageNumber;system.loadTorrentToList({node:node,page:p})}if(currentTorrentId!=0)system.control.torrentlist.datagrid("selectRecord",currentTorrentId);
system.reloading=false;if(system.config.autoReload)system.autoReloadTimer=setTimeout(function(){system.reloadData()},system.config.reloadStep);system.updateTreeNodeText("torrent-all",system.lang.tree.all+this.showNodeMoreInfos(transmission.torrents.count,transmission.torrents.totalSize));var items="uploadedBytes,downloadedBytes,filesAdded,sessionCount,secondsActive".split(",");$.each(items,function(key,item){switch(item){case "uploadedBytes":case "downloadedBytes":system.updateTreeNodeText(item,system.lang.tree.statistics[item]+
formatSize(system.serverSessionStats["cumulative-stats"][item]));system.updateTreeNodeText("current-"+item,system.lang.tree.statistics[item]+formatSize(system.serverSessionStats["current-stats"][item]));break;case "secondsActive":system.updateTreeNodeText(item,system.lang.tree.statistics[item]+getTotalTime(system.serverSessionStats["cumulative-stats"][item]*1E3));system.updateTreeNodeText("current-"+item,system.lang.tree.statistics[item]+getTotalTime(system.serverSessionStats["current-stats"][item]*
1E3));break;default:system.updateTreeNodeText(item,system.lang.tree.statistics[item]+system.serverSessionStats["cumulative-stats"][item]);system.updateTreeNodeText("current-"+item,system.lang.tree.statistics[item]+system.serverSessionStats["current-stats"][item]);break}});for(var index in transmission.torrents.folders){var item=transmission.torrents.folders[index];oldInfos.folders[item.nodeid]=null}this.loadFolderList(oldInfos.folders);if(navigator.userAgent.indexOf("Firefox")>0)system.panel.left.find("span.nav-total-size").css({"margin-top":"-19px"})},
showNodeMoreInfos:function(count,size){var result="";if(count>0)result=" \x3cspan class\x3d'nav-torrents-number'\x3e("+count+")\x3c/span\x3e";if(size>0)result+="\x3cspan class\x3d'nav-total-size'\x3e["+formatSize(size)+"]\x3c/span\x3e";return result},getServerStatus:function(){if(this.reloading)return;clearTimeout(this.autoReloadTimer);this.reloading=true;transmission.getStatus(function(data){system.reloading=false;$("#status_downloadspeed").html(formatSize(data["downloadSpeed"],false,"speed"));$("#status_uploadspeed").html(formatSize(data["uploadSpeed"],
false,"speed"));system.serverSessionStats=data;if(data["torrentCount"]==0){var parentNode=system.panel.left.tree("find","servers");if(parentNode)system.panel.left.tree("remove",parentNode.target);system.updateTreeNodeText("torrent-all",system.lang.tree.all)}})},showStatus:function(msg,outtime){if($("#m_status").panel("options").collapsed)$("#layout_left").layout("expand","south");this.panel.status_text.show();this.panel.status_text.html(msg);if(outtime==0)return;if(outtime==undefined)outtime=3E3;
this.panel.status_text.fadeOut(outtime,function(){$("#layout_left").layout("collapse","south")})},updateTreeNodeText:function(id,text,iconCls){var node=this.panel.left.tree("find",id);if(node){var data={target:node.target,text:text};if(iconCls!=undefined)data["iconCls"]=iconCls;this.panel.left.tree("update",data)}node=null},appendTreeNode:function(parentid,data){var parent=null;if(typeof parentid=="string")parent=this.panel.left.tree("find",parentid);else parent=parentid;if(parent)this.panel.left.tree("append",
{parent:parent.target,data:data});else this.panel.left.tree("append",{data:data});parent=null},removeTreeNode:function(id){var node=this.panel.left.tree("find",id);if(node)this.panel.left.tree("remove",node.target);node=null},loadTorrentToList:function(config){if(!transmission.torrents.all)return;var def={node:null,page:1};jQuery.extend(def,config);if(!config.node)return;var torrents=null;var parent=this.panel.left.tree("getParent",config.node.target)||{id:""};var currentNodeId=this.panel.left.data("currentNodeId");
if(currentNodeId!=config.node.id){this.control.torrentlist.datagrid({pageNumber:1});currentNodeId=config.node.id}this.panel.left.data("currentNodeId",currentNodeId);switch(parent.id){case "servers":torrents=transmission.trackers[config.node.id].torrents;break;default:switch(config.node.id){case "torrent-all":case "servers":torrents=transmission.torrents.all;break;case "paused":torrents=transmission.torrents.status[transmission._status.stopped];break;case "sending":torrents=transmission.torrents.status[transmission._status.seed];
break;case "seedwait":torrents=transmission.torrents.status[transmission._status.seedwait];break;case "check":torrents=transmission.torrents.status[transmission._status.check];break;case "checkwait":torrents=transmission.torrents.status[transmission._status.checkwait];break;case "downloading":torrents=transmission.torrents.status[transmission._status.download];break;case "downloadwait":torrents=transmission.torrents.status[transmission._status.downloadwait];break;case "actively":torrents=transmission.torrents.actively;
break;case "error":torrents=transmission.torrents.error;break;case "warning":torrents=transmission.torrents.warning;break;case "search-result":torrents=transmission.torrents.searchResult;break;default:if(config.node.id.indexOf("folders-")!=-1){var folder=transmission.torrents.folders[config.node.id];if(folder)torrents=folder.torrents}break}break}if(this.config.defaultSelectNode!=config.node.id){this.control.torrentlist.datagrid("loadData",[]);this.config.defaultSelectNode=config.node.id;this.saveConfig()}var datas=
new Array;for(var index in torrents){if(!torrents[index])return;var status=this.lang.torrent["status-text"][torrents[index].status];var percentDone=parseFloat(torrents[index].percentDone*100).toFixed(2);if(status==transmission._status.check)percentDone=parseFloat(torrents[index].recheckProgress*100).toFixed(2);if(torrents[index].error!=0)status="\x3cspan class\x3d'text-status-error'\x3e"+status+"\x3c/span\x3e";else if(torrents[index].warning)status="\x3cspan class\x3d'text-status-warning' title\x3d'"+
torrents[index].warning+"'\x3e"+status+"\x3c/span\x3e";var data={};data=$.extend(data,torrents[index]);data.status=status;data.statusCode=torrents[index].status;data.completeSize=Math.max(0,torrents[index].totalSize-torrents[index].leftUntilDone);data.leecherCount=torrents[index].leecher;data.seederCount=torrents[index].seeder;datas.push(data)}this.updateTorrentCurrentPageDatas(datas)},updateTorrentCurrentPageDatas:function(currentTypeDatas){var rows=this.control.torrentlist.datagrid("getRows");if(currentTypeDatas.length==
0&&rows.length>0){this.control.torrentlist.datagrid("loadData",[]);return}var _options=this.control.torrentlist.datagrid("options");var orderField=null;if(_options.sortName){orderField=_options.sortName;var orderField_func=orderField;if(orderField=="remainingTime")orderField_func="remainingTimeRaw";currentTypeDatas=currentTypeDatas.sort(arrayObjectSort(orderField_func,_options.sortOrder))}if(rows.length==0||currentTypeDatas.length!=this.control.torrentlist.datagrid("getData").total&&currentTypeDatas.length>
_options.pageSize){this.control.torrentlist.datagrid({loadFilter:pagerFilter,pageNumber:_options.pageNumber,sortName:orderField,sortOrder:_options.sortOrder}).datagrid("loadData",currentTypeDatas);return}this.control.torrentlist.datagrid("getData").originalRows=currentTypeDatas;var start=(_options.pageNumber-1)*parseInt(_options.pageSize);var end=start+parseInt(_options.pageSize);currentTypeDatas=currentTypeDatas.slice(start,end);var recently={};var datas={};for(var index in transmission.torrents.recently){var item=
transmission.torrents.recently[index];recently[item.id]=true;item=null}for(var index in currentTypeDatas){var item=currentTypeDatas[index];datas[item.id]=item;item=null}var addedDatas={};for(var index=rows.length-1;index>=0;index--){var item=rows[index];var data=datas[item.id];if(!data)this.control.torrentlist.datagrid("deleteRow",index);else if(recently[item.id]){this.control.torrentlist.datagrid("updateRow",{index:index,row:data});addedDatas[item.id]=item}else if(transmission.torrents.removed)if(transmission.torrents.removed.length>
0&&$.inArray(item.id,transmission.torrents.removed)!=-1)this.control.torrentlist.datagrid("deleteRow",index);else addedDatas[item.id]=item;else addedDatas[item.id]=item;item=null;data=null}for(var index in currentTypeDatas){var item=currentTypeDatas[index];if(!addedDatas[item.id])this.control.torrentlist.datagrid("appendRow",item)}rows=null;recently=null;datas=null},getTorrentNameBar:function(torrent){var className="";var tip=torrent.name;switch(torrent.status){case transmission._status.stopped:className=
"iconlabel icon-pause-small";break;case transmission._status.check:className="iconlabel icon-checking";break;case transmission._status.download:className="iconlabel icon-down";break;case transmission._status.seed:className="iconlabel icon-up";break;case transmission._status.seedwait:case transmission._status.downloadwait:case transmission._status.checkwait:className="iconlabel icon-wait";break}tip+="\n"+torrent.downloadDir;if(torrent.warning){className="iconlabel icon-warning-type1";tip+="\n\n"+this.lang["public"]["text-info"]+
": "+torrent.warning}if(torrent.error!=0){className="iconlabel icon-exclamation";tip+="\n\n"+this.lang["public"]["text-info"]+": "+torrent.errorString}return'\x3cspan class\x3d"'+className+'" title\x3d"'+tip+'"\x3e'+torrent.name+"\x3c/span\x3e"},getTorrentProgressBar:function(progress,torrent){progress=progress+"%";var className="";var status=0;if(typeof torrent=="object")status=torrent.status;else status=torrent;switch(status){case transmission._status.stopped:className="torrent-progress-stop";break;
case transmission._status.checkwait:case transmission._status.check:className="torrent-progress-check";break;case transmission._status.downloadwait:case transmission._status.download:className="torrent-progress-download";break;case transmission._status.seedwait:case transmission._status.seed:className="torrent-progress-seed";break}if(typeof torrent=="object"){if(torrent.warning)className="torrent-progress-warning";if(torrent.error!=0)className="torrent-progress-error"}return'\x3cdiv class\x3d"torrent-progress" title\x3d"'+
progress+'"\x3e\x3cdiv class\x3d"torrent-progress-text"\x3e'+progress+'\x3c/div\x3e\x3cdiv class\x3d"torrent-progress-bar '+className+'" style\x3d"width:'+progress+';"\x3e\x3c/div\x3e\x3c/div\x3e'},addTorrentsToServer:function(urls,count,autostart,savepath){var index=count-urls.length;var url=urls.shift();if(!url){this.showStatus(this.lang.system.status.queuefinish);this.getServerStatus();return}this.showStatus(this.lang.system.status.queue+(index+1)+"/"+count+"\x3cbr/\x3e"+url,0);transmission.addTorrentFromUrl(url,
savepath,autostart,function(data){system.addTorrentsToServer(urls,count,autostart,savepath)})},changeSelectedTorrentStatus:function(status,button,method){var rows=this.control.torrentlist.datagrid("getChecked");var ids=new Array;if(!status)status="start";for(var i in rows)ids.push(rows[i].id);if(!method)method="torrent-"+status;if(ids.length>0){if(button){var icon=button.linkbutton("options").iconCls;button.linkbutton({disabled:true,iconCls:"icon-loading"})}transmission.exec({method:method,arguments:{ids:ids}},
function(data){if(button)button.linkbutton({iconCls:icon});system.control.torrentlist.datagrid("uncheckAll");system.reloadTorrentBaseInfos()})}},searchTorrents:function(key){if(key=="")return;var result=transmission.torrents.search(key);if(result==null||result.length==0){this.removeTreeNode("search-result");return}var node=this.panel.left.tree("find","search-result");var text=this.lang.tree["search-result"]+" : "+key+" ("+result.length+")";if(node==null){this.appendTreeNode("torrent-all",[{id:"search-result",
text:text,iconCls:"icon-search"}]);node=this.panel.left.tree("find","search-result")}else this.panel.left.tree("update",{target:node.target,text:text});this.panel.left.tree("select",node.target)},getTorrentInfos:function(id){if(!transmission.torrents.all[id])return;if(transmission.torrents.all[id].infoIsLoading)return;if(this.currentTorrentId>0&&transmission.torrents.all[this.currentTorrentId])if(transmission.torrents.all[this.currentTorrentId].infoIsLoading)return;this.currentTorrentId=id;if(!this.panel.attribute.panel("options").collapsed){var torrent=
transmission.torrents.all[id];torrent.infoIsLoading=true;var fields="fileStats,trackerStats,peers,leftUntilDone,status,rateDownload,rateUpload,uploadedEver,uploadRatio,error,errorString";if(!torrent.moreInfosTag)fields+=",files,trackers,comment,dateCreated,creator,downloadDir";transmission.torrents.getMoreInfos(fields,id,function(result){torrent.infoIsLoading=false;if(result==null)return;jQuery.extend(torrent,result[0]);if(system.currentTorrentId==0||system.currentTorrentId!=id){system.clearTorrentAttribute();
return}torrent.completeSize=torrent.totalSize-torrent.leftUntilDone;torrent.moreInfosTag=true;system.fillTorrentBaseInfos(torrent);system.fillTorrentFileList(torrent);system.fillTorrentServerList(torrent);system.fillTorrentPeersList(torrent);system.fillTorrentConfig(torrent);transmission.torrents.all[id]=torrent;transmission.torrents.datas[id]=torrent})}},clearTorrentAttribute:function(){system.panel.attribute.find("#torrent-files-table").datagrid("loadData",[]);system.panel.attribute.find("#torrent-servers-table").datagrid("loadData",
[]);system.panel.attribute.find("#torrent-peers-table").datagrid("loadData",[]);system.panel.attribute.find("span[id*\x3d'torrent-attribute-value']").html("")},updateCurrentPageDatas:function(keyField,datas,sourceTable){var rows=sourceTable.datagrid("getRows");var _options=sourceTable.datagrid("options");var orderField=null;if(_options.sortName){orderField=_options.sortName;datas=datas.sort(arrayObjectSort(orderField,_options.sortOrder))}if(rows.length==0||datas.length!=sourceTable.datagrid("getData").total&&
datas.length>_options.pageSize){sourceTable.datagrid({loadFilter:pagerFilter,pageNumber:1,sortName:orderField,sortOrder:_options.sortOrder}).datagrid("loadData",datas);return}sourceTable.datagrid("getData").originalRows=datas;var start=(_options.pageNumber-1)*parseInt(_options.pageSize);var end=start+parseInt(_options.pageSize);datas=datas.slice(start,end);var newDatas={};for(var index in datas){var item=datas[index];newDatas[item[keyField]]=item;item=null}for(var index=rows.length-1;index>=0;index--){var item=
rows[index];var data=newDatas[item[keyField]];if(data)sourceTable.datagrid("updateRow",{index:index,row:data});else sourceTable.datagrid("deleteRow",index);data=null;item=null}},fillTorrentBaseInfos:function(torrent){$.each(torrent,function(key,value){switch(key){case "rateDownload":case "rateUpload":value=formatSize(value,true,"speed");break;case "totalSize":case "uploadedEver":case "leftUntilDone":case "completeSize":value=formatSize(value);break;case "addedDate":case "dateCreated":case "doneDate":value=
formatLongTime(value);break;case "status":value=system.lang.torrent["status-text"][value];break;case "error":if(value==0)system.panel.attribute.find("#torrent-attribute-tr-error").hide();else system.panel.attribute.find("#torrent-attribute-tr-error").show();break;case "comment":value=system.replaceURI(value);break}system.panel.attribute.find("#torrent-attribute-value-"+key).html(value)})},fillTorrentFileList:function(torrent){var files=torrent.files;var fileStats=torrent.fileStats;var datas=new Array;
var namelength=torrent.name.length+1;for(var index in files){var file=files[index];var stats=fileStats[index];var percentDone=parseFloat(stats.bytesCompleted/file.length*100).toFixed(2);datas.push({name:file.name==torrent.name?file.name:file.name.substr(namelength),index:index,bytesCompleted:stats.bytesCompleted,percentDone:system.getTorrentProgressBar(percentDone,transmission._status.download),length:file.length,wanted:system.lang.torrent.attribute["status"][stats.wanted],priority:'\x3cspan class\x3d"iconlabel icon-flag-'+
stats.priority+'"\x3e'+system.lang.torrent.attribute["priority"][stats.priority]+"\x3c/span\x3e"})}this.updateCurrentPageDatas("index",datas,system.panel.attribute.find("#torrent-files-table"))},fillTorrentServerList:function(torrent){var trackers=torrent.trackers;var trackerStats=torrent.trackerStats;var datas=new Array;for(var index in trackers){var item=trackers[index];var stats=trackerStats[index];var rowdata={};for(var key in stats)switch(key){case "lastAnnounceTime":case "nextAnnounceTime":rowdata[key]=
formatLongTime(stats[key]);break;case "lastAnnounceSucceeded":case "lastAnnounceTimedOut":rowdata[key]=system.lang.torrent.attribute["status"][stats[key]];break;default:rowdata[key]=stats[key];break}datas.push(rowdata)}transmission.torrents.addTracker(torrent);this.updateCurrentPageDatas("id",datas,system.panel.attribute.find("#torrent-servers-table"))},fillTorrentPeersList:function(torrent){var peers=torrent.peers;var datas=new Array;for(var index in peers){var item=peers[index];var rowdata={};for(var key in item)rowdata[key]=
