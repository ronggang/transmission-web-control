...
labels:!1},labels:[],labelMaps:{}},storageKeys:{dictionary:{folders:"dictionary.folders"}},dictionary:{folders:null},checkUpdateScript:"https://raw.githubusercontent.com/ronggang/twc-release/master/update.json",contextMenus:{},panel:null,lang:null,reloading:!1,autoReloadTimer:null,downloadDir:"",islocal:!1,B64:new Base64,currentTorrentId:0,control:{tree:null,torrentlist:null},userConfig:{torrentList:{fields:[],sortName:null,sortOrder:"asc"}},serverConfig:null,serverSessionStats:null,templates:{},
checkedRows:[],uiIsInitialized:!1,popoverCount:0,setlang:function(a,b){a||(a=this.config.defaultLang?this.config.defaultLang:navigator.language||navigator.browserLanguage);a||(a="zh-CN");-1!=a.indexOf("-")&&(a=a.split("-")[0].toLocaleLowerCase()+"-"+a.split("-")[1].toLocaleUpperCase());this.languages[a]||(a="en");$.getScript(system.rootPath+"lang/"+a+".js",function(){system.lang=$.extend(!0,system.defaultLang,system.lang);system.resetLangText();$.getScript(system.rootPath+"script/easyui/locale/easyui-lang-"+
a.replace("-","_")+".js").done(function(a,d){b&&b()}).fail(function(a,d,e){$.getScript(system.rootPath+"script/easyui/locale/easyui-lang-en.js",function(){b&&b()})})})},init:function(a,b,c){this.readConfig();this.lastUIStatus=JSON.parse(JSON.stringify(this.config.ui.status));this.islocal=1==b?!0:!1;this.panel={main:$("#main"),top:$("#m_top"),toolbar:$("#m_toolbar"),left_layout:$("#m_left_layout"),left:$("#m_left"),body:$("#m_body"),layout_body:$("#layout_body"),layout_left:$("#layout_left"),list:$("#m_list"),
...
b.menu({onClick:function(a){"icon-ok"==a.iconCls?(system.control.torrentlist.datagrid("hideColumn",a.name),b.menu("setIcon",{target:a.target,iconCls:"icon-empty"})):(system.control.torrentlist.datagrid("showColumn",a.name),b.menu("setIcon",{target:a.target,iconCls:"icon-ok"}));system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.saveUserConfig()}});for(var a=system.control.torrentlist.datagrid("getColumnFields"),c=0;c<a.length;c++){var f=a[c],g=
system.control.torrentlist.datagrid("getColumnOption",f);0!=g.allowCustom&&"false"!=g.allowCustom&&b.menu("appendItem",{text:g.title,name:f,iconCls:g.hidden?"icon-empty":"icon-ok"})}}this.control.torrentlist=$("\x3ctable/\x3e").attr("class","torrent-list").appendTo(this.panel.list);var b=null,c=-1;$.get(system.rootPath+"template/torrent-fields.json?time\x3d"+new Date,function(d){d=d.fields;for(var e={},f=0;f<d.length;f++){var g=d[f];e[g.field]=g}0!=system.userConfig.torrentList.fields.length&&(d=
$.extend(d,system.userConfig.torrentList.fields));system.userConfig.torrentList.fields=d;for(var h in d)g=d[h],(f=e[g.field])&&f.formatter?g.formatter=f.formatter:g.formatter&&delete g.formatter,g.title=system.lang.torrent.fields[g.field]||g.field,system.setFieldFormat(g);system.control.torrentlist.datagrid({autoRowHeight:!1,pagination:system.config.pagination,rownumbers:!0,remoteSort:!1,checkOnSelect:!1,pageSize:system.config.pageSize,pageList:system.config.pageList,idField:"id",fit:!0,striped:!0,
sortName:system.userConfig.torrentList.sortName,sortOrder:system.userConfig.torrentList.sortOrder,drophead:!0,columns:[d],onCheck:function(a,b){system.checkTorrentRow(a,b)},onUncheck:function(a,b){system.checkTorrentRow(a,b)},onCheckAll:function(a){system.checkTorrentRow("all",!1)},onUncheckAll:function(a){system.checkTorrentRow("all",!0)},onSelect:function(a,b){-1!=c&&system.control.torrentlist.datagrid("unselectRow",c);system.getTorrentInfos(b.id);c=a},onUnselect:function(a,b){system.currentTorrentId=
0;c=-1},onBeforeLoad:function(a){system.currentTorrentId=0},onSortColumn:function(a,b){var c=system.control.torrentlist.datagrid("getData").originalRows.sort(arrayObjectSort(a,b));system.control.torrentlist.datagrid("loadData",c);system.resetTorrentListFieldsUserConfig(system.control.torrentlist.datagrid("options").columns[0]);system.userConfig.torrentList.sortName=a;system.userConfig.torrentList.sortOrder=b;system.saveUserConfig()},onRowContextMenu:function(a,b,c){system.config.simpleCheckMode&&
...
e.torrents}else if(-1!=a.node.id.indexOf("label-"))for(e in d=parseInt(a.node.labelIndex),b=[],transmission.torrents.all){var g=transmission.torrents.all[e];(c=this.config.labelMaps[g.hashString])&&-1!=$.inArray(d,c)&&b.push(g)}}}this.config.defaultSelectNode!=a.node.id&&(this.control.torrentlist.datagrid("loadData",[]),this.config.defaultSelectNode=a.node.id,this.saveConfig());a=[];for(f in b){if(!b[f])return;c=this.lang.torrent["status-text"][b[f].status];parseFloat(100*b[f].percentDone).toFixed(2);
c==transmission._status.check&&parseFloat(100*b[f].recheckProgress).toFixed(2);0!=b[f].error?c="\x3cspan class\x3d'text-status-error'\x3e"+c+"\x3c/span\x3e":b[f].warning&&(c="\x3cspan class\x3d'text-status-warning' title\x3d'"+b[f].warning+"'\x3e"+c+"\x3c/span\x3e");e={};e=$.extend(e,b[f]);e.status=c;e.statusCode=b[f].status;e.completeSize=Math.max(0,b[f].totalSize-b[f].leftUntilDone);e.leecherCount=b[f].leecher;e.seederCount=b[f].seeder;if(c=this.config.labelMaps[e.hashString])e.labels=c;a.push(e)}this.updateTorrentCurrentPageDatas(a)}},
...
a);else{this.control.torrentlist.datagrid("getData").originalRows=a;d=(c.pageNumber-1)*parseInt(c.pageSize);c=d+parseInt(c.pageSize);a=a.slice(d,c);c={};d={};for(var e in transmission.torrents.recently){var f=transmission.torrents.recently[e];c[f.id]=!0}for(e in a)f=a[e],d[f.id]=f;var g={};for(e=b.length-1;0<=e;e--){f=b[e];var h=d[f.id];h?c[f.id]?(this.control.torrentlist.datagrid("updateRow",{index:e,row:h}),g[f.id]=f):transmission.torrents.removed?0<transmission.torrents.removed.length&&-1!=$.inArray(f.id,
transmission.torrents.removed)?this.control.torrentlist.datagrid("deleteRow",e):g[f.id]=f:g[f.id]=f:this.control.torrentlist.datagrid("deleteRow",e)}for(e in a)f=a[e],g[f.id]||this.control.torrentlist.datagrid("appendRow",f)}}},getTorrentNameBar:function(a){var b="",c=a.name;switch(a.status){case transmission._status.stopped:b="iconlabel icon-pause-small";break;case transmission._status.check:b="iconlabel icon-checking";break;case transmission._status.download:b="iconlabel icon-down";break;case transmission._status.seed:b=
...
(c="torrent-progress-error"));return'\x3cdiv class\x3d"torrent-progress" title\x3d"'+a+'"\x3e\x3cdiv class\x3d"torrent-progress-text"\x3e'+a+'\x3c/div\x3e\x3cdiv class\x3d"torrent-progress-bar '+c+'" style\x3d"width:'+a+';"\x3e\x3c/div\x3e\x3c/div\x3e'},addTorrentsToServer:function(a,b,c,d){var e=b-a.length,f=a.shift();f?(this.showStatus(this.lang.system.status.queue+(e+1)+"/"+b+"\x3cbr/\x3e"+f,0),transmission.addTorrentFromUrl(f,d,c,function(f){system.addTorrentsToServer(a,b,c,d)})):(this.showStatus(this.lang.system.status.queuefinish),
this.getServerStatus())},changeSelectedTorrentStatus:function(a,b,c){var d=this.control.torrentlist.datagrid("getChecked"),e=[];a||(a="start");for(var f in d)e.push(d[f].id);c||(c="torrent-"+a);if(0<e.length){if(b){var g=b.linkbutton("options").iconCls;b.linkbutton({disabled:!0,iconCls:"icon-loading"})}transmission.exec({method:c,arguments:{ids:e}},function(a){b&&b.linkbutton({iconCls:g});system.control.torrentlist.datagrid("uncheckAll");system.reloadTorrentBaseInfos()})}},searchTorrents:function(a){if(""!=
...
navigator.userAgent.indexOf("Firefox")&&system.panel.left.find("span.nav-total-size").css({"margin-top":"-19px"});system.initUIStatus()})},appendFolder:function(a){if(a){var b="folders";a=a.replace(/\\/g,"/").split("/");var c="folders-",d="",e;for(e in a){var f=a[e];if(""!=f){d+=f;var g=this.B64.encode(f);c+=g.replace(/[+|\/|=]/g,"0");g=this.panel.left.tree("find",c);var h=transmission.torrents.folders[c];h?(f+=this.showNodeMoreInfos(h.count,h.size),g?this.updateTreeNodeText(c,f):(this.appendTreeNode(b,
[{id:c,path:d,text:f,iconCls:"iconfont tr-icon-file"}]),"folders"!=b&&(g=this.panel.left.tree("find",b),this.panel.left.tree("collapse",g.target))),b=c):(this.debug("appendFolder:key",c),this.debug("appendFolder:name",f),this.debug("appendFolder:node",g))}}}},replaceURI:function(a){return a.replace(/(http|https|ftp):\/\/([^/:]+)(:\d*)?([^# ]*)/ig,function(a){return'\x3ca href\x3d"'+a+'" target\x3d"_blank"\x3e'+a+"\x3c/a\x3e"})},readConfig:function(){this.readUserConfig();var a=this.getStorageData(this.configHead+
...
this.userConfig=$.extend(!0,this.userConfig,a))},saveUserConfig:function(){window.localStorage[this.configHead]=JSON.stringify(this.userConfig)},uploadTorrentFile:function(a,b,c,d){if(window.FileReader){var e=$("input[id\x3d'"+a+"']")[0].files;$.each(e,function(a,g){transmission.addTorrentFromFile(g,b,c,d,e.length)})}else alert(system.lang["public"]["text-browsers-not-support-features"])},checkUpdate:function(){$.ajax({url:this.checkUpdateScript,dataType:"json",success:function(a){a&&a.update&&(system.codeupdate<
a.update?($("#area-update-infos").show(),$("#msg-updateInfos").html(a.update+" -\x3e "+a.infos)):$("#area-update-infos").hide())}})},changeLanguages:function(a){a!=this.lang.name&&a&&(this.config.defaultLang=a,this.saveConfig(),location.href="?lang\x3d"+a)},getStorageData:function(a,b){return null==window.localStorage[a]?b:window.localStorage[a]},setStorageData:function(a,b){window.localStorage[a]=b},openDialogFromTemplate:function(a){a=$.extend(!0,{id:null,options:null,datas:null,type:0},a);if(null!=
a.id){var b=a.id,c=a.options,d=a.datas,e=$("#"+b);if(e.length){d&&$.each(d,function(a,b){e.data(a,b)});if(0==a.type&&e.attr("type")==a.type){e.dialog("open");e.dialog({content:system.templates[b]});return}if(0!=system.popoverCount){setTimeout(function(){system.openDialogFromTemplate(a)},350);return}e.remove()}c=$.extend(!0,{title:"",width:100,height:100,resizable:!1,cache:!0,content:system.lang.dialog["system-config"].loading,modal:!0},c);e=$("\x3cdiv/\x3e").attr({id:b,type:a.type}).appendTo(document.body);
0==a.type?e.dialog(c):(e.css({width:c.width,height:c.height}).data("popoverSource",a.source),$(a.source).webuiPopover({url:"#"+b,width:c.width,height:c.height-18,padding:!1,onHide:function(c){$(a.source).webuiPopover("destroy");$("#"+b).remove();$(c).remove();system.popoverCount--},onShow:function(){system.popoverCount++}}));$.get(system.rootPath+"template/"+b+".html?time\x3d"+new Date,function(c){system.templates[b]=c;d&&$.each(d,function(a,c){$("#"+b).data(a,c)});0==a.type?$("#"+b).dialog({content:c}):
(e.html(c),$.parser.parse("#"+b),$(a.source).webuiPopover("show"))})}},debug:function(a,b){window.console&&window.console.log&&window.console.log(a,b)},initThemes:function(){this.themes&&$("#select-themes").combobox({groupField:"group",data:this.themes,editable:!1,panelHeight:"auto",onChange:function(a){var b=(a+";").split(";"),c=b[0];b=b[1]||"logo.png";$("#styleEasyui").attr("href","tr-web-control/script/easyui/themes/"+c+"/easyui.css");$("#logo").attr("src","tr-web-control/"+b);system.config.theme=
a;system.saveConfig()},onLoadSuccess:function(){$(this).combobox("setValue",system.config.theme||"default")}})},getValidTreeKey:function(a){return a?this.B64.encode(a).replace(/[+|\/|=]/g,"0"):""}};$(document).ready(function(){$.getScript(system.rootPath+"lang/default.js");$.getScript(system.rootPath+"lang/_languages.js",function(){system.init(location.search.getQueryString("lang"),location.search.getQueryString("local"))})});
