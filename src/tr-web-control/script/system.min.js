...
0!=system.userConfig.torrentList.fields.length&&(r=$.extend(r,system.userConfig.torrentList.fields))
...
s?s.empty():(s=$("<div/>").attr("class","easyui-menu").css({"min-width":"180px"}).appendTo(this.panel.main),this.contextMenus[t]=s,s.menu())
var n=null
...
break
case"_usename_":switch(t.field){case"name":t.formatter=function(t,e,s){return system.getTorrentNameBar(transmission.torrents.all[e.id])}}break
case"ratio":t.formatter=function(t,e,s){var n=""
return parseFloat(t)<1&&(n="text-status-warning"),'<span class="'+n+'">'+t+"</span>"}}},reloadData:function(){this.reloadSession(),this.reloading=!1,this.getServerStatus(),this.reloading=!1,this.reloadTorrentBaseInfos(),this.checkTorrentRow("all",!1)},loadFolderList:function(t){this.removeTreeNode("folders-loading")
...
var t=this.getStorageData(this.configHead+".system")
t&&(this.config=$.extend(this.config,JSON.parse(t)))
for(var e in this.storageKeys.dictionary)this.dictionary[e]=this.getStorageData(this.storageKeys.dictionary[e])},saveConfig:function(){this.setStorageData(this.configHead+".system",JSON.stringify(this.config))
...
$.each(a,function(t,r){transmission.addTorrentFromFile(r,e,s,n,a.length)})}else alert(system.lang.publit["text-browsers-not-support-features"])},checkUpdate:function(){$.ajax({url:this.checkUpdateScript,dataType:"json",success:function(t){t&&t.update&&(system.codeupdate<t.update?($("#area-update-infos").show(),$("#msg-updateInfos").html(t.update+" -> "+t.infos)):$("#area-update-infos").hide())}})},changeLanguages:function(t){t!=this.lang.name&&t&&(this.config.defaultLang=t,this.saveConfig(),location.href="?lang="+t)},getStorageData:function(t,e){return null==window.localStorage[t]?e:window.localStorage[t]},setStorageData:function(t,e){window.localStorage[t]=e},openDialogFromTemplate:function(t){var e={id:null,options:null,datas:null}
